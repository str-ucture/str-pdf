...
if sys.platform == 'win32':
    ...
    if not libgs:
        # try to find the DLL in the Ghostscript installation directory
        # import os
        # gs_path = os.path.join(os.environ['ProgramFiles'], 'gs')
        # for root, dirs, files in os.walk(gs_path):
        #     for f in files:
        #         if f.lower() == 'gsdll64.dll' or f.lower() == 'gsdll32.dll':
        #             libgs = os.path.join(root, f)
        #             break
        #     if libgs:
        #         break
        # libgs = r"Ghostscript\App\bin\gsdll64.dll"
        libgs = r"./Ghostscript/App/bin/gsdll64.dll"
